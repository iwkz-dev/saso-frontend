...
export const cartSlice = createSlice({
  ...
  reducers: {
        ...
        if (itemIndex >= 0) {
          ...
          if (amount <= quantityLeft) {
            ...
            state.data.items[itemIndex].sumPrice = parseFloat(
              (state.data.items[itemIndex].menu.price * amount).toFixed(2)
            );
          }
        } else {
    ...
    removeOrder: (state, action) => {
      const id = action.payload._id;
      const itemIndex = state.data.items.findIndex(item => item.menu._id == id);
      if (itemIndex >= 0) {
        const amount = state.data.items[itemIndex].amount - 1;
        if (amount == 0) {
          ...
          if (isConfirm) {
            const items = state.data.items.filter(item => item.menu._id != id);
