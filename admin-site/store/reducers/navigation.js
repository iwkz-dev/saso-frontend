...
export const navSlice = createSlice({
    ...
    reducers: {
        updatePage: (state, action) => {
            ...
            const getLastPageIndex = state.navigation.findIndex(nav => nav.current === true);
            nav[getLastPageIndex].current = false;
            const getCurrentPageIndex = state.navigation.findIndex(nav => nav.name === action.payload.name);
